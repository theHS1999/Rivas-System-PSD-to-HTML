...
@section('content')
    <ol class="breadcrumb" >
        <li class="active bold">ویرایش پزشک</li>
    ...
    <div class="col-sm-8 col-sm-offset-2">
        ...
        <div class="form-group col-xs-6">
            <label>نام </label>
            <input type="text" class="form-control" name="fname" value="{{$doctor->fname}}" required>
        </div>
        <div class="form-group col-xs-6">
            <label>نام خانوادگی</label>
            <input type="text" class="form-control" name="lname" value="{{$doctor->lname}}" required>
        </div>
        <div class="form-group col-xs-6">
            <label>کد نظام پزشکی</label>
            <input type="text" class="form-control" name="medical_code" value="{{$doctor->medical_code}}" required>
        </div>
        <div class="form-group col-xs-6">
            <label>تخصص</label>
            <select type="text" class="form-control select2" name="expertise" >
                @foreach($expertises as $expertise)
                    <option value="{{$expertise->id}}" {{$expertise->id==$doctor->expertise ? 'selected' : ''}}>{{$expertise->name}}</option>
                @endforeach
            </select>
        </div>
